...
export async function createPickupGame(input: {
  ...
  if (!Number.isInteger(winThreshold) || winThreshold < 25 || winThreshold > 200) {
  ...
  if (guestNames.some((name) => name.length > 50)) throw new Error("Guest names must be 50 characters or fewer");

  const game = await prisma.$transaction(async (tx) => {
    let created;
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        created = await tx.game.create({
          data: {
            kind: "PICKUP",
            startedAt: null,
            hostUserId: host.id,
    ...
    if (!created) throw new Error("Unable to create a unique lobby code");

    const guests = [];
    for (const name of guestNames) {
      guests.push(await tx.guestUser.create({ data: { name, createdById: host.id } }));
    }
    const colorInputs = [
    ...
    await tx.gamePlayers.createMany({
      data: [
        { gameId: created.id, userId: host.id, accentColor: colors[host.id] },
        ...guests.map((guest) => ({ gameId: created!.id, guestId: guest.id, accentColor: colors[guest.id] })),
      ],
    });
    return created;
  });
  ...
  posthog.capture({
    ...
    properties: { game_id: game.id, guest_player_count: guestNames.length, win_threshold: winThreshold },
  });
...
export async function joinPickupGame(joinToken: string) {
  ...
  const gameId = await prisma.$transaction(async (tx) => {
    ...
    const game = await tx.game.findUnique({
      where: { id: rows[0].id },
      include: { players: { include: { user: { select: { accentColor: true } } } } },
    });
    if (!game || game.kind !== "PICKUP" || game.startedAt || game.isFinished) {
      throw new Error("This lobby is no longer accepting players");
    }
    if (game.players.some((player) => player.userId === joiningUser.id)) return game.id;
    ...
    await tx.gamePlayers.create({
      data: { gameId: game.id, userId: joiningUser.id, accentColor: colors[key] },
    });
    return game.id;
  });

  posthog.capture({ distinctId: user.userId, event: "join_pickup_game", properties: { game_id: gameId } });
...
export async function joinPickupGameByCode(rawCode: string) {
  ...
  const game = await prisma.game.findUnique({ where: { joinCode: code }, select: { joinToken: true } });
...
export async function startPickupGame(gameId: string) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (game.hostUserId !== host.id) throw new Error("Only the host can start this game");
    if (game.startedAt) return;
    if (game._count.players < 2) throw new Error("At least two players are needed to start");
    await tx.game.update({
      where: { id: gameId },
      data: { startedAt: new Date(), joinToken: null, joinCode: null },
    });
  });
  posthog.capture({ distinctId: user.userId, event: "start_pickup_game", properties: { game_id: gameId } });
