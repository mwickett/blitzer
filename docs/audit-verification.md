...
- Retired `OrganizationMembership` and guest invitation columns remain in the schema. Caller review found no live app use; the destructive drop is deferred pending owner inventory — see [schema cleanup](./schema-cleanup.md). The older broad event-instrumentation wishlist in #217 and the separate Insights/marketing product work are not completed by this audit stack.
