...
import { seedGameData, type SeedConfig, type SyncedUser } from "./fixtures";
...
async function main() {
  ...
  if (!clerkSecretKey?.startsWith("sk_test_")) {
    throw new Error("A Clerk development secret key is required for seeding");
  }
  ...
  try {
    ...
    // Step 3: Initialize missing fixtures without resetting tester changes
