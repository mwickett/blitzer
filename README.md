...
Clerk is the source of Circle membership. The old `OrganizationMembership` table and guest invitation columns remain in the schema; the application does not synchronize or use them. A 2026-09-24 review confirmed no live app callers and **deferred the drop** until an owner inventories production data — see [schema cleanup](docs/schema-cleanup.md). The retired contact export and invitation migration must not be restored. Removing those files did not erase copies in historical commits or external artifacts. Ops follow-up for Slack secrets and contact-export artifacts: [docs/external-cleanup.md](docs/external-cleanup.md).
